...
;; Alternative function to help manually import failed PSDs using IMPORTSTYLESANDSETTINGS
; (defun c:PSDIMPORT ( / srcPath oldCmdEcho oldCmdDia)
  ; (princ "\n=== Manual PSD Import Helper ===")
  ; (princ "\nThis command will attempt to use IMPORTSTYLESANDSETTINGS for manual import.")
  
  ; (setq srcPath (getfiled "Select DWT template file" "" "dwt" 0))
  
  ; (if srcPath
    ; (progn
      ; (princ "\nAttempting to use IMPORTSTYLESANDSETTINGS command...")
      ; (princ "\nIf this command is not available, use Format > Style Manager manually.")
      
      ; Save current system variables
      ; (setq oldCmdEcho (getvar "CMDECHO"))
      ; (setq oldCmdDia (getvar "CMDDIA"))
      
      ; Enable dialogs for user interaction
      ; (setvar "CMDECHO" 1)
      ; (setvar "CMDDIA" 1)
      
      ; Try the import command
      ; (if (vl-catch-all-error-p (vl-catch-all-apply 'command (list "IMPORTSTYLESANDSETTINGS" srcPath)))
        ; (progn
          ; (princ "\nIMPORTSTYLESANDSETTINGS command not available.")
          ; (princ "\nPlease use Format > Style Manager to manually import PSDs:")
          ; (princ (strcat "\n1. Go to Format > Style Manager"))
          ; (princ (strcat "\n2. Right-click on drawing name and select 'Open'"))
          ; (princ (strcat "\n3. Navigate to: " srcPath))
          ; (princ (strcat "\n4. Expand Property Set Definitions"))
          ; (princ (strcat "\n5. Copy failed PSDs manually"))
        ; )
        ; (princ "\nIMPORTSTYLESANDSETTINGS dialog should be open. Select Property Set Definitions to import.")
      ; )
...
;; Diagnostic command to analyze specific PSDs and their properties
; (defun c:PSDDIAGNOSE ( / *error* app docs dstDoc srcPath srcDoc srcDicts psdNames psdName psdObj)
  ...
  ;; Analyze a single PSD object
  ; (defun analyzePSD (psdObj psdName / propCount propDef i appToObjs propName propType dataType dataFormat)
    ; (princ (strcat "\n\n=== Analyzing PSD: " psdName " ==="))
    
    ; Basic info
    ; (princ (strcat "\nObject Type: " (vla-get-objectname psdObj)))
    ; (princ (strcat "\nDescription: " (vl-catch-all-apply 'vla-get-description (list psdObj))))
    
    ; Applied to objects
    ; (setq appToObjs (vl-catch-all-apply 'vla-get-appliedto (list psdObj)))
    ; (if (not (vl-catch-all-error-p appToObjs))
      ; (progn
        ; (princ "\nApplied to objects:")
        ; (vlax-for appObj appToObjs
          ; (princ (strcat "\n  - " (vl-catch-all-apply 'vla-get-name (list appObj))))
        ; )
      ; )
      ; (princ "\nApplied to: Unable to retrieve")
    ; )
    
    ; Property definitions
    ; (setq propCount (vl-catch-all-apply 'vla-get-count (list (vla-get-propertydefinitions psdObj))))
    ; (if (not (vl-catch-all-error-p propCount))
      ; (progn
        ; (princ (strcat "\nProperty Count: " (itoa propCount)))
        ; (setq i 0)
        ; (while (< i propCount)
          ; (setq propDef (vl-catch-all-apply 'vla-item (list (vla-get-propertydefinitions psdObj) i)))
          ; (if (not (vl-catch-all-error-p propDef))
            ; (progn
              ; (setq propName (vl-catch-all-apply 'vla-get-name (list propDef)))
              ; (setq propType (vl-catch-all-apply 'vla-get-datatype (list propDef)))
              ; (setq dataFormat (vl-catch-all-apply 'vla-get-dataformat (list propDef)))
              ; (princ (strcat "\n  Property " (itoa (1+ i)) ": " 
                           ; (if (vl-catch-all-error-p propName) "Unknown" propName)))
              ; (princ (strcat "\n    Data Type: " 
                           ; (if (vl-catch-all-error-p propType) "Unknown" (itoa propType))))
              ; (princ (strcat "\n    Data Format: " 
                           ; (if (vl-catch-all-error-p dataFormat) "None" 
                               ; (vla-get-name dataFormat))))
            ; )
          ; )
          ; (setq i (1+ i))
        ; )
      ; )
      ; (princ "\nProperty definitions: Unable to retrieve")
    ; )
    
    ; Check for extensions/custom data
    ; (princ "\nExtended Data:")
    ; (if (vlax-property-available-p psdObj 'XData)
      ; (progn
        ; (setq xdata (vl-catch-all-apply 'vla-GetXData (list psdObj "" 'xtype 'xdata)))
        ; (if (and (not (vl-catch-all-error-p xdata)) xtype)
          ; (princ (strcat "\n  Has XData: " (itoa (vlax-safearray-get-u-bound xtype 1))))
          ; (princ "\n  No XData")
        ; )
      ; )
      ; (princ "\n  XData not accessible")
    ; )
  ; )
  ...
  ; (setq app     (vlax-get-acad-object)
        ...
        ; dstDoc  (vla-get-activedocument app)
        ; srcPath (getfiled "Select DWT template file" "" "dwt" 0)
        ; psdNames '("Decke" "Fenster" "hsbPlatte" "hsbStab" "Treppe" "Türen" "Wand")
  ; )
  ...
  ;; Open source file in read-only mode
  ; (princ (strcat "\nOpening template file for diagnosis: " srcPath))
  ; (setq srcDoc (vla-open docs srcPath :vlax-true))
  ; (if (not srcDoc)
    ; (progn (princ "\nError: Cannot open template file.") (exit))
  ; )

  ;; Get Property Set Definitions dictionary
  ; (setq srcDicts (vl-catch-all-apply 'vla-item (list (vla-get-dictionaries srcDoc) "AEC_PROPERTY_SET_DEFS")))
  
  ; (if (vl-catch-all-error-p srcDicts)
    ; (princ "\nError: AEC_PROPERTY_SET_DEFS dictionary not found in template file.")
    ; (progn
      ; (princ "\n\n=== DIAGNOSING FAILED PSDs ===")
      ; (foreach psdName psdNames
        ; (setq psdObj (vl-catch-all-apply 'vla-item (list srcDicts psdName)))
        ; (if (vl-catch-all-error-p psdObj)
          ; (princ (strcat "\n" psdName ": Not found in template"))
          ; (analyzePSD psdObj psdName)
        ; )
      ; )
      
      ; (princ "\n\n=== DEPENDENCY CHECK ===")
      ; (princ "\nChecking for data format dependencies...")
      ; (setq formatDict (vl-catch-all-apply 'vla-item (list (vla-get-dictionaries srcDoc) "AEC_PROPERTY_DATA_FMT_DEFS")))
      ; (if (vl-catch-all-error-p formatDict)
        ; (princ "\nNo AEC_PROPERTY_DATA_FMT_DEFS found")
        ; (progn
          ; (princ "\nProperty Data Formats available:")
          ; (vlax-for fmt formatDict
            ; (princ (strcat "\n  - " (vla-get-name fmt)))
          ; )
        ; )
      ; )
...
; (princ "\n  PSDIMPORT  - Helper for manual PSD import using Style Manager")
; (princ "\n  PSDDIAGNOSE - Analyze specific PSDs and their properties")
; (princ "\n")
; (princ) 
